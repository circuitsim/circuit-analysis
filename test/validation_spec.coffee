	...
	describe 'assert', ->
		...
		describe 'notNegative', ->
			...
			it 'should throw an exeption if <0', ->
				expect(-> assert(-1).notNegative()).to.throw("ValidationException: Expected >0. Was: -1")
		...
		describe 'isNumber', ->
			...
			it 'should throw an exception if given a string', ->
				expect(-> assert("5").isNumber()).to.throw("ValidationException: Expected number. Was string: 5")
		...
		describe 'isArray', -> 
			...
			it 'should throw an exception if given an object', ->
				expect(-> assert({}).isArray()).to.throw("ValidationException: Expected array. Was object: [object Object]")

		describe 'lessThan', ->
			it 'should not throw an exception when <', ->
				expect(-> assert(1).lessThan(2)).to.not.throw()
			it 'should throw an exception when equal', ->
				expect(-> assert(1).lessThan(1)).to.throw("Expected <1. Was: 1")
			it 'should throw an exception when >', ->
				expect(-> assert(2).lessThan(1)).to.throw("Expected <1. Was: 2")